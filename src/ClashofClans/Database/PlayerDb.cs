    ...
    public class PlayerDb
    {
        ...
        public static async Task<Player> CreateAsync()
        {
            ...
            try
            {
                ...
                using (var cmd =
                    new MySqlCommand(
                        $"INSERT INTO {Name} (`Id`, `IsOnline`, `Trophies`, `PreviousSeasonMonth`, `PreviousSeasonTrophies`, `Language`, `FacebookId`, `Home`, `Objects`, `Sessions`) " +
                        $"VALUES ({id + 1}, {home.Status}, {home.Trophies}, {home.PreviousSeasonMonth}, {home.PreviousSeasonTrophies}, @language, @fb, @home, @objects, @sessions)")
                )
                {
...
#pragma warning restore 618
        ...
        public static async Task SaveAsync(Player player)
        {
            ...
            try
            {
                ...
                using (var cmd =
                    new MySqlCommand(
                        $"UPDATE {Name} SET `IsOnline`='{home.Status}', `Trophies`='{home.Trophies}', " +
                        $"`PreviousSeasonMonth`='{home.PreviousSeasonMonth}', `PreviousSeasonTrophies`='{home.PreviousSeasonTrophies}', " +
                        $"`Language`=@language, `FacebookId`=@fb, `Home`=@home, `Objects`=@objects, `Sessions`=@sessions WHERE Id = '{home.Id}'")
                )
                {
#pragma warning disable 618
                    cmd.Parameters?.AddWithValue("@language", home.PreferredDeviceLanguage);
